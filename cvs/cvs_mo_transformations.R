...
prepare_p1_ClaimBaseTableDF <- function(iwdfs) {
  ...
  iwdfs.V_PHMCY_CLM <- aliasDF(iwdfs.V_PHMCY_CLM, "PHMCY")
  ...
  iwdfs.V_MBR_ACCT_HIST <- aliasDF(iwdfs.V_MBR_ACCT_HIST, "MBR_ACC")
  #add non null filter
  ...
  iwdfs.V_DRUG_DENORM <- aliasDF(iwdfs.V_DRUG_DENORM, "DRUG")
...
prepare_p1_maxFillDtTable <- function(claimBaseTable){
  ...
  maxFillDtTable <- aliasDF(maxFillDtTable, "MFDT")
...
maxClmEventGidTable <- aliasDF(maxClmEventGidTable, "MCEGT")
