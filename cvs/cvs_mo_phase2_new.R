...
prepareMoCampaignTable <- function() {
  ...
  transformedTable <- iwSelect(
    completeTable, "*",
      ...
      #MBR_CURR_AGE
      paste("FLOOR(MONTHS_BETWEEN('", currentDate, "', MBR_ACC_MBR_BRTH_DT) / 12) as MBR_CURR_AGE", sep = ""),
      #MINOR_IND
      "CASE WHEN MBR_CURR_AGE < 18 THEN 'Y' ELSE 'N' END AS MINOR_IND",
      #MBR_CURR_ELIG_IND
      paste("CASE WHEN '", currentDate , "' BETWEEN MBR_ACC_CVRG_CVRG_EFF_DT and MBR_ACC_CVRG_CVRG_EXPRN_DT THEN 'Y' ELSE 'N' END AS MBR_CURR_ELIG_IND", sep = ""),
      #RTM_IND TODO add for DLVRY_SYSTM_CD from phmcy claim
      "CASE WHEN BNFT_PLAN_RCP_OPTNS_MAIL_IND = 'Y' THEN 'Y' ELSE 'N' END AS RTM_IND",
      #MCV_IND
      ...
      "CASE WHEN DRUG_DEA_CLS_CD IN ('0',' ') THEN 'N' ELSE 'Y' END AS CONTROLLED_SUBSTANCE_IND"
  )
